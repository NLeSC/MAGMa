...
public class JobDescriptionFactoryTest extends TestCase {
    ...
    public void testGetJobDescriptionSleep() {
        ...
        JobSubmitRequest jobsubmission = new JobSubmitRequest("/somepath",
                "sleep");
        try {
            JobDescription jd = fact.getJobDescription(jobsubmission);
            assertEquals(jd.getSoftwareDescription().getExecutable(),
                    "/bin/sleep");
    ...
    @Test
    public void testGetJobDescriptionMzxmlLocal() throws Exception {
        JobDescriptionFactory fact = new JobDescriptionFactory();
        ...
        // 1. run mscore_mzxml with parameters in config file and files in
        // jobdir
        ...
        String jobdir = System.getProperty("java.io.tmpdir") + "/"
        ...
        JobSubmitRequest jobsubmission = new JobSubmitRequest(jobdir,
                "mzxmllocal", arguments);
        JobDescription jd = fact.getJobDescription(jobsubmission);

        SoftwareDescription sd = jd.getSoftwareDescription();
        assertEquals(sd.getExecutable(), "/bin/sh");
        String[] jobargs = { "mscore_mzxml-local.sh", "allinone", "-p",
                arguments.precision, "-c", arguments.mscutoff, "-d",
                arguments.msmscutoff, "-i", arguments.ionisation, "-n",
                arguments.nsteps, "-b", arguments.phase,
                jobdir + "/data.mzxml", jobdir + "/smiles.txt",
                jobdir + "/results.db" };
    ...
    @Test
    public void testGetJobDescriptionMzxmlRemote() throws Exception {
        JobDescriptionFactory fact = new JobDescriptionFactory();

        // When user submits a magma job the website will put
        // the user supplied mzxml file, smiles file and config file into a
        // jobdir.
        // then do a rest call to this service
        ...
        JobSubmitRequest jobsubmission = new JobSubmitRequest(jobdir,
                "mzxmlremote", arguments);
        JobDescription jd = fact.getJobDescription(jobsubmission);

        SoftwareDescription sd = jd.getSoftwareDescription();
        assertEquals(sd.getExecutable(), "/bin/sh");
        String[] jobargs = { "mscore_mzxml.sh", "allinone", "-p",
                arguments.precision, "-c", arguments.mscutoff, "-d",
                arguments.msmscutoff, "-i", arguments.ionisation, "-n",
                arguments.nsteps, "-b", arguments.phase, "data.mzxml",
                "smiles.txt", "results.db" };
        assertArrayEquals(sd.getArguments(), jobargs);
        assertEquals(sd.getStdout().getAbsolutePath(), jobdir + "/stdout.txt");
        assertEquals(sd.getStderr().getAbsolutePath(), jobdir + "/stderr.txt");
        /**
         * assert pre and post staged files Pre: { "Magma-1.1.tar.gz": null,
         * "mscore_mzxml.sh": null, jobdir+"data.mzxml": null,
         * jobdir+"smiles.sd": null }
         *
         * Post: { "results.db": jobdir+"/results.db" }
         */
        HashMap<File, File> expectedPreStaged = new HashMap<File, File>();
        ...
        expectedPostStaged.put(GAT.createFile("results.db"),
                GAT.createFile(jobdir + "/results.db"));
