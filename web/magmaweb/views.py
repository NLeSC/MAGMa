...
class JobViews(object):
    ...
    @view_config(route_name='molecules.json', renderer='json')
    def moleculesjson(self):
        """Returns json document with molecules,
    ...
    @view_config(route_name='molecules.csv')
    def moleculescsv(self):
        """Same as :func:`moleculesjson`, but returns csv file
        ...
        """
        mets = self.moleculesjson()
        cols = []
        if ('cols' in self.request.params):
            cols = json.loads(self.request.params['cols'])
        csv = self.job.db.molecules2csv(mets['rows'], cols=cols)
    ...
    @view_config(route_name='molecules.sdf')
    def moleculessdf(self):
        """Same as :func:`moleculesjson`, but returns sdf file
        ...
        if ('cols' in self.request.params):
            cols = json.loads(self.request.params['cols'])
        sdf = self.job.db.molecules2sdf(mets['rows'], cols=cols)
    ...
    def extractedionchromatogram(self):
        ...
        for a molecule and the id,rt of scans which have molecule hits

        request.matchdict['molid']
            Molecule identifier
        ...
        """
        molid = self.request.matchdict['molid']
        return {
            'chromatogram': self.job.db.extractedIonChromatogram(molid),
            'scans': self.job.db.scansWithMolecules(molid=molid)
        }

    @view_config(route_name='fragments.json', renderer='json')
    def fragments(self):
        """Returns json object with molecules
        ...
        ``molid``
            Fragments of molecule with this identifier
