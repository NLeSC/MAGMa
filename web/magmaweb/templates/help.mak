...
<html>
  ...
  <body>
    ...
    <ul>
      <li>mzXML: a public dataformat that can be generated with
        <ul>
          ...
          <li><a href="http://proteowizard.sourceforge.net/downloads.shtml">MSConvert</a>
            module from ProteoWizard</li>
        </ul>
      </li>
      <li>Mass Tree: a custom format to enter a single spectral tree based on
        m/z, e.g.
    ...
    MS2_peakX_mz: MS2_peakX_intenstity
      ...
      <li>Formula Tree: a custom format to enter a single spectral tree based on
        molecular formulas, e.g.
    ...
    <ul>
      ...
      <li>MS1 scan number: Only read spectral tree specified by MS1 scan number.<br>
        Note: the public website requires specification of one scan if
        candidates are retrieved from a database.</li>
    </ul>
    When importing mzXML data, please:<br>
    <ul>
      <li><b>Use centroid data!</b> (MAGMa considers all signals to be different
        ions.)</li>
      <li>Note that when multiple scans are provided for the same precursor (in
        the same precursor scan) MAGMa merges these scans into a single
        composite spectrum which includes only the most intense peak for a given
        m/z, using the given precision to match peaks from the different scans.</li>
    </ul>
    <ul>
    </ul>
    ...
    <ul>
      <li>Retrieval from a Database (PubChem, Kegg or Human Metabolite
        Database). Precursor ion m/z values of all MS2 scans are used to query
        the database on mono-isotopic mass, assuming [M-H]<sup>-</sup> or [M+H]<sup>+</sup>
        ions, depending on the specified ionisation mode.</li>
      <li>Upload from smiles or SDF (Structure Data File format).</li>
      <li>Drawing a molecule.</li>
    </ul>
    Metabolizing molecules:<br>
    <br>
    Uploaded or drawn molecules can be submitted to <i>in silico</i> metabolite
    prediction to augment the set of candidate molecules. MAGMa metabolite
    prediction is based on reaction rules that are applied according to a
    specified editable "scenario". The buttons "Drugs" and "Polyphenols" provide
    predefined scenario's that can be used to generate human metabolites for
    these classes of compounds. Phase1 and phase2 human biotransformation rules
    are derived according to the method outlined in [<a href="http://dx.doi.org/10.1002/cmdc.200700312">Ridder
      and Wagener 2008</a>]. Phase1_selected and phase2_selected are subsets
    including the most common rules: more than 5% of the metabolites they
    predict for a large and diverse set of clinically studied compounds agreed
    with the clinical observations. The "glycosidase" and "gut" rules are
    described in [Ridder et al. submitted]. The first set of rules defined in
    the scenario is applied to all uploaded or drawn molecule(s). Subsequent
    stages are also applied to products of the preceding steps. When a mass
    filter is applied, the subsequent stage in the scenario are performed only
    on molecules below the specified mass limit. The value in the "Steps" column
    determines either the number of reaction steps to be applied with the
    specified set of rules, or the mass limit in case of a mass filter step. The
    glycosidase rule can also be used with the word "complete", which will
    iterate the glycosidase rule until no further metabolites are generated. In
    ...
    <ul>
      ...
      <li>Accuracy:
        <ul>
          ...
          <li>Precursor m/z (Da): This is used to match precursor ion m/z values
            with the corresponding peaks in the parent spectrum</li>
        </ul>
      </li>
      <li>Intensity thresholds:
        <ul>
          <li>MS<sup>1</sup> (abs): can be used to only annotate the most
            intense peaks in the Chromatogram</li>
          <li>MS<sup>n&gt;1</sup> (% of base peak): threshold to filter fragment
            ion peaks</li>
        </ul>
      </li>
    </ul>
    For more information on the parameters used in the automatic annotation
    please refer to [<a href="http://dx.doi.org/10.1002/rcm.6364">Ridder et al.
      2012</a>].<br>
    <br>
    <br>
    ...
    <ul>
      ...
      <li>When you have imported LC-MSn data:
        <ul>
          <li>The Scans column indicates the number of MS2 scans with matching
            precursor m/z values in the data</li>
          <li>The Assigned column indicates if you have assigned this molecule
            to a precursor ion in your dataset (the “assign molecule” button is
            explained below) </li>
        </ul>
      </li>
      <li>By clicking a row of the table, you can select a candidate molecule.
        In case of LC-MSn data:
        <ul>
          <li>The Chromatogram panel will show an extracted ion chromatogram for
            the corresponding m/z </li>
          <li>The precursor peaks (i.e. the corresponding precursor scans)
            matching the candidate molecule are labeled with green triangles in
            the Chromatogram panel </li>
        </ul>
      </li>
      <li>When <i>in silico</i> metabolite prediction has been applied, the
        Reactions column provides information about the number of connected
        reactant and product molecules. Numbers in bold/italic indicate that
        these include metabolites which match one or more precursor peaks in the
        data. When clicking this information the molecules are filtered to
        display the actual reactant or product structures. When using this
        reactant or product filter, it is recommended to turn off other filters
        on the molecules table.</li>
    ...
    <ul>
      ...
      <li>A spectral tree can be selected by clicking one of the green
        triangles, as a result:
        <ul>
          <li>the Molecules table will show only candidate molecules that match
            the selected spectral tree </li>
          <li>two new columns appear in the Molecules table:
            <ul>
              <li> Candidate (penalty) score: lower values indicate more likely
                candidates</li>
              <li> ΔMass indicates the deviation between the calculated and the
                observed m/z value</li>
            </ul>
          </li>
        </ul>
      </li>
    ...
    peak. The assignment can be undone by clicking the button again. The
