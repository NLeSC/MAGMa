...
from magma.models import DBSession, Base, Metabolite, Scan, Peak, Fragment, Run
...
precision=1.00001 # =10 ppm precision to identify masses
...
#MSMSfilter = 0.0 # Intensity cutoff (relative to basePeakIntensity) to select fragment peaks
pp = 1e-7        # precision for matching precursor mz
...
class scantype:
   def __init__(self,scan):
      ...
      if scan.mslevel==1:
         ...
         cutoff=scan.basepeakintensity*MSMSfilter
      ...
      for childscan in childscans:
         for peak in self.peaks:
            if -pp < childscan.precursormz-peak.mz < pp:
            ...
            if childscan.precursorintensity > cutoff:
               ...
               self.peaks[-1].childscan=scantype(childscan)
...
def add_run_data(nsteps,usephase1,usephase2,MSfilename,ionisation,useFragments,\
                 MSfilter,MSMSfilter,precision,useMSMSonly):
   global sth
   run=Run(n_reaction_steps=nsteps,
           use_phase1=usephase1,
           use_phase2=usephase2,
           ms_filename=MSfilename,
           ionisation=ionisation,
           use_fragmentation=useFragments,
           ms_intensity_cutoff=MSfilter,
           msms_intensity_cutoff=MSMSfilter,
           mz_precision=precision,
           use_msms_only=useMSMSonly
           )
   sth.add(run)
...
def add_metabolite(mol,name,prob,level,sequence,isquery):
   ...
   try:
      ...
      if dupid.probability < prob:
         ...
         sth.commit()
         sth.add(metab)
         print 'Duplicate structure:',sequence,smiles,' - old one removed'
      else:
         print 'Duplicate structure:',sequence,smiles,' - kept old one'
...
def metabolize(metid,metabolism,nsteps):
   ...
   exec_reactor=os.path.join(current_dir,"script","reactor")+" -as -m "+str(nsteps)
   metabolism_files={
      "1": os.path.join(current_dir, "data", "sygma_rules.phase1.smirks"),
      "2": os.path.join(current_dir, "data", "sygma_rules.phase2.smirks")
      }
   usephase={'1':0,'2':0}
   for m in metabolism:
...
def storeMZxmlScan(mzxmlScan,precScan,namespace):
   ...
   scan=Scan(
      ...
      rt=float(mzxmlScan.attrib['retentionTime'].strip('PTS')),
...
def searchMetabolite(metabolite):
   ...
   def fragment2atoms(fragment):
      atomstring=''
      for atom in range(len(Atombits)):
         if (Atombits[atom] & fragment):
            atomstring+=','+str(atom)
      return(atomstring[1:])

   def Score(fragment):
      exit('Using wrong function')
      score = 0
      for bond in range(len(currentBondbits)):
         if 0 < (fragment & Bondbits[bond]) < Bondbits[bond]:
            score += Bondscore[bond]
            # self.bonds.append(bond)
            # self.allbonds=self.allbonds|currentBondbits[bond] # set all bits of broken bond atoms
      return score
         ...
         if not (useMSMSonly and peak.childscan==None):
            ...
            if type(deltaH)==int:
               if not Fragmented:
                  ...
                  sys.stderr.write('Metabolite '+str(metabolite.metid)+': '+str(metabolite.origin)+' -> '+str(metabolite.reactionsequence)+'\n')
                  ...
                  for fragment in FragmentFormID:
                     if FragmentFormID[fragment][1]<=(Nbonds+1):
